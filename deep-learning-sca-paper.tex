...
Side Channel Analysis (henceforth SCA) refers to a technique used to break encryption schemes by using data indirectly related to the cryptographic algorithm. Even a theoretically perfectly safe cryptographic algorithm can be subject to SCA and be broken by it. SCA uses data such as power consumption of an IC, electro magnetic emissions or even emitted sound to reconstruct the entire or parts of the key used to encrypt the data. Therefore, to protect against SCA it is vital to pay attention not only on the theoretical safty of an encryption algorithm but also on its implementation.
...
\subsection{Where Machine Learning used in Template Attacks}
